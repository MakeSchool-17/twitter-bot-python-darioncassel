...
class MarkovModel:
    ...
    def train(self, words, n):
        ...
        for index in range(words_length):
            ...
            for i in range(n):
                if i == n-1:
                    current_words += words[index+i].lower()
                else:
                    current_words += words[index+i].lower() + " "
            next_word = words[index+n].lower()
            if not next_word == current_words.split(" ")[-1]:
                if not self.pointers.get(next_word, None):
                    self.pointers[next_word] = None
                if not self.pointers.get(current_words, None):
                    self.pointers[current_words] = None
                    heap = SortedSinglyLinkedList()
                    heap.insert(next_word)
    ...
    def random_walk(self, length):
        ...
        while length > 0:
            ...
            current_words = self.pointers[current_words]
            if current_words:
